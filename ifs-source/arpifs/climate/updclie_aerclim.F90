SUBROUTINE UPDCLIE_AERCLIM(YDGEOMETRY,YDDYNA,YDCOMPO, YDMCC, YDGFL, YDML_GCONF,PTSTEP)
...
!     Reads the time-varying 3D fields for composition.  
...
!ASSOCIATE(NACTAERO=>YGFL%NACTAERO, NCHEM=>YGFL%NCHEM, YCHEM=>YGFL%YCHEM, NGHG=>YGFL%NGHG,LINJ=>YRCOMPO%LINJ, &
ASSOCIATE(NDGLG=>YDDIM%NDGLG, NDGNH=>YDDIM%NDGNH, NDLON=>YDDIM%NDLON, &
 ...
 & NDIFC_COMPO=>YDMCC%NDIFC_COMPO, NJDCR_COMPO=>YDMCC%NJDCR_COMPO,NYSDMP_AERCLIM=>YDMCC%NYSDMP_AERCLIM, &
...
!*    2.2 READ, DECODE HEADERS, CHECK THE FIELDS AND POSITIONS THE FILE
    ...
    SCAN: DO JTIM=1,10000
      ...
      DO JLEV=1,NFLEVG
        ...
        WRITE(NULOUT,'(A,I6,A,I4,A,I8.8,A)')&
...
!* Decide if reading time
  ...
  WRITE(NULOUT,*) 'UPDCLIME_AERCLIM IDATE INFO: ',IDATE,IDATEREF,LLREAD, NJDCR_COMPO,NDIFC_COMPO
...
!VH Strange/problematic line?!          CALL GRIB_SET(IGRIB,'missingValue',19591204.0_JPRB)
          ...
          WRITE(NULOUT,'(A,I6,A,I8.8,A, I4, A)')&
        ...
        !! WRITE(NULERR,"(A,4I6)"),'SHAPE CLIMRAER:',SHAPE(CLIMRAER(:,1:NFLEVG,NPCOMPO_2,JCL))
        !! WRITE(NULERR,"(A,2I6)"),'other dims:',YDGEOMETRY%YRGEM%NGPTOTG, YDGEOMETRY%YRGEM%NGPTOT

        ! DM communications.
        IF (MYPROC == 1) THEN
             CALL DISGRID_SEND(YDGEOMETRY,NFLEVG,ZBUF_2D,JCL,CLIMRAER(:,1:NFLEVG,NPCOMPO_2,JCL))
        ELSE
             !CALL DISGRID_RECV(YDGEOMETRY%YRGEM,1,NFLEVG,CLIMRAER(:,1:NFLEVG,NPCOMPO_2,JCL),JCL)
             CALL DISGRID_RECV(YDGEOMETRY,1,NFLEVG,CLIMRAER(:,1:NFLEVG,NPCOMPO_2,JCL),JCL)
        ENDIF
   ...
   DO IPARMAL=1,NAERCLIM
     ...
     IF (LMCCIEC_AERCLIM) THEN
       ...
       DO JSTGLO=1,NGPTOT,NPROMA
         ...
         DO JROF =1,IEND
           YDGFL%GFL(JROF,JLEV,IYSDMP,IBL)=CLIMRAER(JSTGLO+JROF-1,JLEV,NPCOMPO_1,IPARMAL)*ZPOID1 &
           & +CLIMRAER(JSTGLO+JROF-1,JLEV,NPCOMPO_2,IPARMAL)*ZPOID2
         ENDDO
        ENDDO 
        ENDDO
     ELSE
       ...
       DO JSTGLO=1,NGPTOT,NPROMA
         ...
         IBL=(JSTGLO-1)/NPROMA+1
         DO JROF =1,IEND
           YDGFL%GFL(JROF,JLEV,IYSDMP,IBL)=CLIMRAER(JSTGLO+JROF-1,JLEV,NPCOMPO_2,IPARMAL)
         ENDDO
       ENDDO 
       ENDDO
     ENDIF  
...
WRITE(NULOUT,*) 'UPDCLIE_AERCLIM finished'
CALL FLUSH(NULOUT)

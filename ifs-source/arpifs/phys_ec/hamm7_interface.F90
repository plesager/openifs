...
REAL(KIND=JPRB) :: ZTSO2(KLON, KLEV)  , ZTSO4(KLON, KLEV), ZTSO4_AQ(KLON, KLEV)
...
! & NDDUST=>YREAERSRC%NDDUST, NTYPAER=>YREAERSRC%NTYPAER, &
 ...
 & RSIGAIR=>YRERAD%RSIGAIR,YAEROCLIM=>YGFL%YAEROCLIM, & !!! YAERCLIM is now become YAEROCLIM 
 & NSTART=>YDRIP%NSTART,AERO_SCHEME=>YRCOMPO%AERO_SCHEME,& !CHEM_SCHEME=>YDCHEM%CHEM_SCHEME,&
...
!-- the total extinction coefficient at wavelengths ?? nm is archived in GFL%AEROUT
  ...
  IF(.NOT.LIFSMIN  .AND. .NOT.LIFSTRAJ) THEN
     ...
     if (.not.LAERCHEM)THEN
        ...
        PGFL(KIDIA:KFDIA,1:KLEV,YAEROUT(43)%MP)=ZTSO4(KIDIA:KFDIA,1:KLEV) ! tendency SS CS ham after update surface

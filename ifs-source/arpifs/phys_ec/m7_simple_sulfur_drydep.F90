...
REAL(KIND=JPRB)   ,INTENT(IN)    :: PAERO(KLON,KLEV,YGFL%NACTAERO), PTAERI(KLON,KLEV,YGFL%NACTAERO)
!REAL(KIND=JPRB)   ,INTENT(IN)    :: PAPH(KLON,0:KLEV)
!REAL(KIND=JPRB)   ,INTENT(IN)    :: PAP(KLON,KLEV)
REAL(KIND=JPRB)   ,INTENT(IN)    :: PDP(KLON,KLEV), PGEOH(KLON,0:KLEV), PRHO(KLON,KLEV)
REAL(KIND=JPRB)   ,INTENT(IN)    :: PFAERI(KLON,YGFL%NACTAERO)
...
!---output fields
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PFAERO(KLON,YGFL%NACTAERO)
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PTAERO(KLON,KLEV,YGFL%NACTAERO)
...
REAL(KIND=JPRB) :: ZVDEP,ZVDEP2(YGFL%NACTAERO)
...
ASSOCIATE(YGFL=>YDMODEL%YRML_GCONF%YGFL, YREAERATM=>YDMODEL%YRML_PHY_RAD%YREAERATM, &
     YREAERSRC=>YDMODEL%YRML_PHY_AER%YREAERSRC, YREAERSNK=>YDMODEL%YRML_PHY_AER%YREAERSNK, &
     YDRIP=>YDMODEL%YRML_GCONF%YRRIP )

ASSOCIATE(NDRYDEP=>YREAERATM%NDRYDEP, &

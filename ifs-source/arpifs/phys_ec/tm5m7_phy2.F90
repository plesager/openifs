SUBROUTINE TM5M7_PHY2 &
 ...
 &  PSO2L, PSO2H, PSO2DD, PSOGF, PVOLC , PVOLE , PSOA, PSOACO, PDMS, PSOIL_TYPE, &
 &  PCI  ,PCLAKE, PINJF, PBLH, PFRTI, PLSM , PSST, PQP   , PSNS  , &
...
!-- OUTPUTS
 ...
 &  PDMSI, PODMS, PEXTRA , PSO4SRC, PSO2SRC)
...
REAL(KIND=JPRB),INTENT(IN)    :: PVOLC(KLON), PVOLE(KLON), PSOA(KLON), PSOACO(KLON)
REAL(KIND=JPRB),INTENT(IN)    :: PDMS(KLON), PSOIL_TYPE(KLON)  
...
REAL(KIND=JPRB),INTENT(OUT)   :: PAERDDP(KLON,YDMODEL%YRML_GCONF%YGFL%NACTAERO), PAERSDM(KLON,YDMODEL%YRML_GCONF%YGFL%NACTAERO), PAERSRC(KLON,YDMODEL%YRML_GCONF%YGFL%NACTAERO)
...
ASSOCIATE(NACTAERO=>YGFL%NACTAERO, NAERO=>YGFL%NAERO, &
 ...
 & NDRYDEP=>YDEAERATM%NDRYDEP, NINDSEDM=>YDEAERATM%NINDSEDM, &
 & NTSEDM=>YDEAERATM%NTSEDM, &
 & LCHEM_JOUT=>YDCHEM%LCHEM_JOUT, &
 & NTYPAER=>YDEAERSRC%NTYPAER, AERO_SCHEME=>YDCOMPO%AERO_SCHEME, &
 & NSTART=>YDRIP%NSTART, LCHEM_DIA=>YDCOMPO%LCHEM_DIA, NCHEM=>YGFL%NCHEM , YAERO=>YGFL%YAERO)
!     ------------------------------------------------------------------
...
!end DO
  CALL TM5M7_SRC &
    ...
    &  PBCBF  , PBCFF , PBCGF, POMBF, POMFF, POMGF, &
    &  PRS1   , PRSF1 , PCI  , PCLAKE, PINJF, PBLH, ZDP, PGELAM, PGELAT, PGEMU, PFRTI, PHSDFOR, &
    &  PLSM   , PSST  , PQP   , ZRHO , PSNS , PTP  , PTL  , PTSPHY, PZ0M, KCHEM, &
    &  ZWND   , PWSA1 , PSOIL_TYPE, &
    &  PCVL  , PCVH, KTVL, KTVH, &
    &  PLDAY  , PAERFLX, PCFLX, PCEN , PTENC, ZEMIDIAG, PSO2SRC,PSO4SRC,PSOA,PSOACO)  

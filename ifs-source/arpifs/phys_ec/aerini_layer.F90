...
!VH  IF(.not. LAERCHEM .and. LAEROSFC)THEN
!VH   !IF(LAEROSFC)
!VH    call simple_sulfur_src(YDGEOMETRY, YDMODEL, KDIM%KIDIA, KDIM%KFDIA, KDIM%KLON , KDIM%KTDIA, KDIM%KLEV,& 
!VH         &  KDIM%KSTGLO, GEMSL%ITRAC, GEMSL%IAERO, &
!VH         &  PAUX%PAPHI, &
!VH         &  PSURF%PSD_VF(:,YSD_VF%YSO2L%MP), PSURF%PSD_VF(:,YSD_VF%YSO2H%MP), &
!VH         &  PSURF%PSD_VF(:,YSD_VF%YSOGF%MP),&
!VH         &  PSURF%PSD_VF(:,YSD_VF%YSOA%MP) ,&
!VH         &  PSURF%PSD_VF(:,YSD_VF%YSOACO%MP),PSURF%PSD_VF(:,YSD_VF%YVOLC%MP), PSURF%PSD_VF(:,YSD_VF%YVOLE%MP),PSURF%PSD_VF(:,YSD_VF%YDMSO%MP),&
!VH         &  PSURF%PSD_VF(:,YSD_VF%YCI%MP)  , PSURF%PSD_VF(:,YSD_VF%YINJF%MP) , PSURF%PSD_VD(:,YSD_VD%YBLH%MP) ,&
!VH         &  PAUX%PRS1, PAUX%PRSF1,PAUX%PGELAM, PAUX%PGELAT,&
!VH         &  PSURF%PSD_VF(:,YSD_VF%YLSM%MP) ,  PSURF%PSP_RR(:,YSP_RR%YT%MP9)  , TSPHY,&
!VH         &  GEMSL%ZAERWS,&
!VH         &  GEMSL%ZDMSO, GEMSL%ZLDAY, GEMSL%ZLISS, GEMSL%ZSO2, GEMSL%ZTDMS,&
!VH         &  GEMSL%ZODMS, PSO4SRC,PSO2SRC)
!VH ELSE
   ! Set to zero 
...
!VH    CALL TM5M7_PHY2 &
!VH        &( YDGEOMETRY, YDMODEL, KDIM%KIDIA, KDIM%KFDIA, KDIM%KLON , KDIM%KTDIA, KDIM%KLEV, KDIM%KFLDX , KDIM%KLEVX,& 
!VH        &  KDIM%KTILES, KDIM%KSTGLO, GEMSL%ITRAC, GEMSL%IAERO, NSW,&
!VH        &  PAUX%PRS1 , PAUX%PRSF1, PAUX%PAPHI, STATE%T , PAUX%PVERVEL, GEMSL%ZCEN  , PAUX%PGEOMH,&
!VH        &  PSURF%PSD_VD(:,YSD_VD%YALB%MP), SURFL%ZALBD, PSURF%PSD_VF(:,YSD_VF%YALUVD%MP),&
!VH        &  PSURF%PSD_VF(:,YSD_VF%YAERDEP%MP),PSURF%PSD_VF(:,YSD_VF%YAERLTS%MP),PSURF%PSD_VF(:,YSD_VF%YAERSCC%MP),&
!VH        &  GEMSL%ZAERWS, GEMSL%ZAERGUST, GEMSL%ZAERUST,&
!VH        &  PSURF%PSD_VF(:,YSD_VF%YSO2DD%MP),&
!VH        &  PSURF%PSD_VF(:,YSD_VF%YSOGF%MP),&
!VH        &  PSURF%PSD_VF(:,YSD_VF%YSOILTYPE%MP), &
!VH        !TB added lake cover: YCLK, !!!PSP_SG 2 dims becomes now 3 dims
!VH        &  PSURF%PSD_VF(:,YSD_VF%YCI%MP), PSURF%PSD_VF(:,YSD_VF%YCLK%MP) , PSURF%PSD_VF(:,YSD_VF%YINJF%MP) , PSURF%PSD_VD(:,YSD_VD%YBLH%MP) ,&
!VH        &  SURFL%ZFRTI, PSURF%PSD_VF(:,YSD_VF%YLSM%MP) , PSURF%PSD_VF(:,YSD_VF%YSST%MP), STATE%Q, & 
!VH        &  PSURF%PSP_SG(:,1,YSP_SG%YF%MP9) , PSURF%PSP_RR(:,YSP_RR%YT%MP9)  , PAUX%PGELAM, PAUX%PGELAT, PAUX%PGEMU, SURFL%ZHSDFOR,&
!VH        &  STATE%U(:,KDIM%KLEV) , STATE%V(:,KDIM%KLEV) , PSURF%PSP_SB(:,1,YSP_SB%YQ%MP9), TSPHY, GEMSL%ZAZ0M,&
!VH        &  GEMSL%ICHEM,&
!VH        !VH - Introduce Land use info...     
!VH        &  PSURF%PCVL, PSURF%PCVH,PSURF%ITVL,PSURF%ITVH, &
!VH        &  PSURF%PAHFSTI, &!!!FLUX%PFTLHEV, &
!VH        !VH - end     
!VH        &  GEMSL%ZCFLX, GEMSL%ZTENC,&
!VH        &  GEMSL%ZLDAY, GEMSL%ZLISS, GEMSL%ZSO2, GEMSL%ZTDMS,&
!VH        &  GEMSL%ZAERDDP, GEMSL%ZAERSDM, GEMSL%ZAERSRC, GEMSL%ZAERMAP, GEMSL%ZAERFLX, GEMSL%ZAERLIF,&

...
IF ( LCHEM_AEROI ) THEN 
  IF (TRIM(AERO_SCHEME)=="aer" )THEN
    ! * from MACC fields
    CALL TM5_MACC_AEROSOL(YDEAERSNK,YDEAERVOL,YDEAERATM, KIDIA,KFDIA,KLON,KLEV, KAERO, &
      &   PRS1   , PAEROP  , ZRHCL   ,   &
      &  ZTAUS_AER,ZTAUA_AER,ZPMAER)
  ELSEIF (TRIM(AERO_SCHEME)=="glomap") THEN
    WRITE(NULERR,*)'This option for AEROSOL_SCHEME is not possible yet',TRIM(AERO_SCHEME)

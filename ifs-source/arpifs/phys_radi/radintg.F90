...
!-- debug
    ...
    IF (TRIM(AERO_SCHEME) == "hamm7" ) THEN
      ...
      DO JAERO=1,YDMODEL%YRML_PHY_RAD%YRERAD%NTSW
        IAE=(JAERO-1)*KLEV
        ZRGP(1:IL,IHAMAEROSSA+IAE:IHAMAEROSSA+IAE+KLEV-1,IB) = MIN(MAX(0._JPRB,PAEROSSA(IBEG:IEND,1:KLEV,JAERO)), 1._JPRB)
      ENDDO

      DO JAERO=1,YDMODEL%YRML_PHY_RAD%YRERAD%NTSW
        IAE=(JAERO-1)*KLEV
        ZRGP(1:IL,IHAMAEROASYM+IAE:IHAMAEROASYM+IAE+KLEV-1,IB) = MIN( MAX(-1._JPRB, PAEROASYM(IBEG:IEND,1:KLEV,JAERO)), 1._JPRB)
      ENDDO

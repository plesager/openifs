...
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PAERMACCS(YDGEOMETRY%YRGEM%NGPTOT,YDMODEL%YRML_PHY_RAD%YRERAD%NTSW) 
...
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PAERTGS(YDGEOMETRY%YRGEM%NGPTOT,KLEV,YDMODEL%YRML_PHY_RAD%YRERAD%NTSW) 
...
REAL(KIND=JPRB)   ,INTENT(IN)    :: PAEROTAU(YDGEOMETRY%YRGEM%NGPTOT,  KLEV,YDMODEL%YRML_PHY_RAD%YRERAD%NTSW)
REAL(KIND=JPRB)   ,INTENT(IN)    :: PAEROSSA(YDGEOMETRY%YRGEM%NGPTOT,  KLEV,YDMODEL%YRML_PHY_RAD%YRERAD%NTSW)
REAL(KIND=JPRB)   ,INTENT(IN)    :: PAEROASYM(YDGEOMETRY%YRGEM%NGPTOT, KLEV,YDMODEL%YRML_PHY_RAD%YRERAD%NTSW)
...
REAL(KIND=JPRB) :: ZAERTAUS(YDMODEL%YRML_PHY_RAD%YRERAD%NRPROMA,KLEV,YDMODEL%YRML_PHY_RAD%YRERAD%NTSW)
REAL(KIND=JPRB) :: ZAERASYS(YDMODEL%YRML_PHY_RAD%YRERAD%NRPROMA,KLEV,YDMODEL%YRML_PHY_RAD%YRERAD%NTSW)
REAL(KIND=JPRB) :: ZAEROMGS(YDMODEL%YRML_PHY_RAD%YRERAD%NRPROMA,KLEV,YDMODEL%YRML_PHY_RAD%YRERAD%NTSW)
...
IF( LLDEBUG )THEN
  ...
  WRITE(NULOUT,'("RADINTG: IHAMAEROAODLW=",I0)')IHAMAEROAODLW
...
!$OMP& PRIVATE(JSTGLO,JK,JSW,JLW,JL,IAE,IBEG,IEND,IL,IB,IARP,JARP,JAERO,&
...
CALL GSTATS(1209,0)
!$OMP  PARALLEL DO SCHEDULE(DYNAMIC,1)&
!$OMP& PRIVATE(JSTGLO,J,JLEV,JSW,JL,IAE,IBEG,IEND,I,I1,I2,IL,IB,LLDOLAST,JAERO,&
...
!*         4.2     CALL TO ACTUAL RADIATION SCHEME
  ...
  ELSE
    ...
    CALL RADIATION_SCHEME &
     & (YDMODEL, 1, IL, NRPROMA, KLEV, IB, IRADAER, &               ! IB required to read stratosospheric aerosols
     &  ZRII0, &
...
CALL GSTATS(1213,0)
!$OMP  PARALLEL DO SCHEDULE(STATIC) &
!$OMP& PRIVATE(JSTGLO,JK,IBEG,IEND,IL,IB,JSW,JL,I,IAUX,&
...
!-- reconstruct Tegen aer SW OD
       DO JSW=1,YDMODEL%YRML_PHY_RAD%YRERAD%NTSW

...
IHAMAEROAOD   =INDRAD(INEXT,NTSW*KLEV,.TRUE.) 
IHAMAEROSSA   =INDRAD(INEXT,NTSW*KLEV,.TRUE.)
IHAMAEROASYM  =INDRAD(INEXT,NTSW*KLEV,.TRUE.)
!IHAMAEROAODLW =INDRAD(STRATO_CMIP6_NTB*KLEV,.TRUE.) 
...
IRE_LIQ = INDRAD(INEXT,KLEV, NCLOUDACT > 0)
IRE_ICE = INDRAD(INEXT,KLEV, NCLOUDACT > 0)
...
!-- debug
  ...
  !A.Laakso
  IF (NSTEP == YDRIP%NSTART) THEN
     DO JAERO=1,YDERAD%NTSW
     IAE=(JAERO-1)*KLEV
     ZRGP(1:IL,IHAMAEROAOD+IAE:IHAMAEROAOD+IAE+KLEV-1,IB)   =REPSCAER 
     ZRGP(1:IL,IHAMAEROSSA+IAE:IHAMAEROSSA+IAE+KLEV-1,IB)   =0._JPRB
     ZRGP(1:IL,IHAMAEROASYM+IAE:IHAMAEROASYM+IAE+KLEV-1,IB) =0._JPRB
     END DO

     !DO JAERO=1,STRATO_CMIP6_NTB
     !IAE=(JAERO-1)*KLEV
     !ZRGP(1:IL,IHAMAEROAODLW+IAE:IHAMAEROAODLW+IAE+KLEV-1,IB) =0._JPRB
     !END DO
  ...
  ELSE
    ...
    DO JAERO=1,YDMODEL%YRML_PHY_RAD%YRERAD%NTSW  
       IAE=(JAERO-1)*KLEV
       write(*,*)  "REPSCAER", REPSCAER
       write(*,*)  "PAEROTAU", PAEROTAU(IBEG:IEND,1:KLEV,JAERO)
       ZRGP(1:IL,IHAMAEROAOD+IAE:IHAMAEROAOD+IAE+KLEV-1,IB)   =MAX(REPSCAER, PAEROTAU(IBEG:IEND,1:KLEV,JAERO))
    ENDDO

    DO JAERO=1,YDMODEL%YRML_PHY_RAD%YRERAD%NTSW
       IAE=(JAERO-1)*KLEV
       ZRGP(1:IL,IHAMAEROSSA+IAE:IHAMAEROSSA+IAE+KLEV-1,IB)   =MAX(0._JPRB,PAEROSSA(IBEG:IEND,1:KLEV,JAERO))
    ENDDO

    DO JAERO=1,YDMODEL%YRML_PHY_RAD%YRERAD%NTSW
       IAE=(JAERO-1)*KLEV
       ZRGP(1:IL,IHAMAEROASYM+IAE:IHAMAEROASYM+IAE+KLEV-1,IB) =MAX(0._JPRB, PAEROASYM(IBEG:IEND,1:KLEV,JAERO))
    ENDDO

    !DO JAERO=1,STRATO_CMIP6_NTB  
    !IAE=(JAERO-1)*KLEV 
    !ZRGP(1:IL,IHAMAEROAODLW+IAE:IHAMAEROAODLW+IAE+KLEV-1,IB) =MAX(0._JPRB, PAEROTAULW(IBEG:IEND,1:KLEV,JAERO))
    !ENDDO
...
!*         4.2     CALL TO ACTUAL RADIATION SCHEME
  ...
  ELSE
    ...
    CALL RADIATION_SCHEME &
     ...
     &  ZRGP(1,ISwDiffuseBand,IB), ZRGP(1,ISwDirectBand,IB),&
     ! OPTIONAL ARGUMENTS
     &  ZRGP(1,IHAMAEROAOD,IB),  ZRGP(1,IHAMAEROSSA,IB), ZRGP(1,IHAMAEROASYM,IB), & ! added for M7 aerosol 

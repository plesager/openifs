...
IF(TRIM(AERO_SCHEME) == "hamm7" .AND. NCLOUDACT > 0 .AND. PRESENT(PRE_LIQ) .AND. PRESENT(PRE_ICE) ) THEN
    ...
    DO JLEV = 1,KLEV
      ...
      DO JAER=1,YRERAD%NTSW  ! SW radiation     
        ...
        STRAT_AEROSOL%SSA_SW(JAER,JLEV,KIDIA:KFDIA) = &
             &ZAODSTRAT(KIDIA:KFDIA,IJLEV,JSW_ORDERED_BIS)-ZAAODSTRAT(KIDIA:KFDIA,IJLEV,JSW_ORDERED_BIS)
  ...
  ELSEIF(YRERAD%LCMIP_STRATAER_CMIP7) THEN
    DO JLEV = 1,KLEV
      ...
      DO JAER=1,YRERAD%NTSW  ! SW radiation     
        ...
        STRAT_AEROSOL%SSA_SW(JAER,JLEV,KIDIA:KFDIA) = &
             &ZAODSTRAT(KIDIA:KFDIA,IJLEV,JAER)-ZAAODSTRAT(KIDIA:KFDIA,IJLEV,JAER)

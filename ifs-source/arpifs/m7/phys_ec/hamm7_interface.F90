...
IF(.NOT.LIFSMIN  .AND. .NOT.LIFSTRAJ) THEN
  ...
  JK = YDMODEL%YRML_GCONF%YGFL%NAERO_WVL_DIAG ! Should check (at setup, not here) that JK+33 <= KLEV
  PGFL(KIDIA:KFDIA,34:JK+33,YAEROUT(1)%MP)= ZAOD_DIAG(KIDIA:KFDIA,1:YDMODEL%YRML_GCONF%YGFL%NAERO_WVL_DIAG)

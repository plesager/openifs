...
SELECT CASE (TRIM(AERO_SCHEME))
  ...
  CASE("hamm7")
    ...
    PGFL(KIDIA:KFDIA,1:KLEV,YICNC%MP9_PH) = ZICNC(KIDIA:KFDIA,1:KLEV) ! no conversion needed: already in #/cm3
...
IF (.NOT. LAERCHEM .AND. LCHEM_DIA) THEN ! As of 2025-01-24, ZTSO4 is filled only if LAERCHEM=F
...
IF(.NOT.LIFSMIN  .AND. .NOT.LIFSTRAJ) THEN
  ...
  PGFL(KIDIA:KFDIA,10:25,YAEROUT(1)%MP)= PAOD_LW(KIDIA:KFDIA,1:16)
